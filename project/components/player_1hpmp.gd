...
func _ready() -> void:
	...
	hpvalue.text = "HP: " + str(bar_owner.health) + "/" + str(bar_owner.max_hp)
	mpvalue.text = "MP: " + str(bar_owner.mana) + "/" + str(bar_owner.mana)
	bar_owner.health_changed.connect(_on_health_changed)
...
func _on_health_changed(player: Player) -> void:
	if bar_owner == player:
		hpvalue.text = "HP: " + str(player.health) + "/" + str(player.max_hp)
